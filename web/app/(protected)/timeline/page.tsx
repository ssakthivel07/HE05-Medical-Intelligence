...
export default async function TimelinePage() {
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-zinc-50 dark:bg-zinc-950">
        <div className="max-w-md w-full p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 text-center space-y-4 shadow-sm">
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            Please sign in to view your unified medical timeline.
          </p>
          <Link
            href="/login"
        ...
        <div className="max-w-2xl mx-auto py-8">
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 sm:p-8 shadow-xs dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
              Please complete your initial profile before accessing the chronological timeline.
            </p>
            <CreateProfileForm />
          </div>
        </div>
  ...
  // Fetch all medical events for active patient
  ...
  try {
    const { data, error } = await supabase
      .from('medical_events')
      .select('id,patient_id,document_id,event_type,event_date,title,description,confidence,is_verified,created_at')
      .eq('patient_id', patient.id)
      .order('event_date', { ascending: false })

    if (error) {
      dbError = 'Could not load medical timeline events.'
    } else if (data) {
      events = data as MedicalEvent[]
    }
  } catch {
    dbError = 'Error connecting to timeline database.'
  }
  ...
  return (
    ...
    >
      ...
      <TimelineClient initialEvents={events} patientName={patient.full_name} />
    </DashboardLayoutClient>
  )
}

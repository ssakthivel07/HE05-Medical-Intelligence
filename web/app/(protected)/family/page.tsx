...
export default async function FamilyPage() {
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-zinc-50 dark:bg-zinc-950">
        <div className="max-w-md w-full p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 text-center space-y-4 shadow-sm">
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            Please sign in to manage family permissions.
          </p>
          <Link
            href="/login"
        ...
        <div className="max-w-2xl mx-auto py-8">
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 sm:p-8 shadow-xs dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
              Please complete your initial profile before managing family permissions.
            </p>
            <CreateProfileForm />
          </div>
        </div>
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('id,patient_id,user_id,permission_level,created_at')
      .eq('patient_id', patient.id)
      .order('created_at', { ascending: false })

    if (error) {
      dbError = 'Could not load family permissions.'
    } else if (data) {
      permissions = data as FamilyPermission[]
    }
  } catch {
    dbError = 'Error connecting to permissions database.'
  }
  ...
  return (
    ...
    >
      ...
      <FamilyClient
        ...
        isOwner={patient.owner_user_id === user.id}
      />
    </DashboardLayoutClient>
  )
}

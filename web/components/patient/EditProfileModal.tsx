...
export default function EditProfileModal({ patient, isOpen, onClose, onSuccess }: EditProfileModalProps) {
  const router = useRouter()
  const [fullName, setFullName] = useState(patient.full_name)
  const [relationship, setRelationship] = useState(patient.relationship || 'self')
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-zinc-200 dark:bg-zinc-900 dark:border-zinc-800 z-10 animate-in fade-in zoom-in-95 duration-150">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-zinc-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-zinc-900 dark:text-white">Edit Profile Details</h3>
              <p className="text-xs text-zinc-500">Update medical and contact parameters</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-3.5 text-sm">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="e.g. Vikram Sharma - +1 (555) 019-2834"
